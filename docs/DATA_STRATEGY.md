...
export class DriverFactory {
  static create(overrides?: Partial<DriverData>): DriverData {
    ...
    return {
      ...
      email: faker.internet.email({ firstName, lastName, provider: 'test-ueline.com' }),
...
export function getEnvConfig() {
  return {
    ...
    odooDatabase: process.env.ODOO_DATABASE || 'hora_db',
...
# Or delete manually
docker exec -it hora-qa-framework-db-1 psql -U odoo -d hora_db -c "DELETE FROM fleet_vehicle WHERE license_plate LIKE 'MD-TEST%'"
```
