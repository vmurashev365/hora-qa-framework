...
curl -X POST http://localhost:8069/web/session/authenticate `
  ...
  -d '{"jsonrpc":"2.0","method":"call","params":{"db":"hora_db","login":"admin","password":"admin"},"id":1}'
```
...
docker exec hora-qa-framework-db-1 psql -U odoo -d hora_db -c "SELECT 1"
```
...
docker exec -it hora-qa-framework-db-1 psql -U odoo -d hora_db -c "DELETE FROM fleet_vehicle WHERE license_plate ~ '^(MD-TEST|MD-TEMP|TEST)';"
```
...
**Maintained By**: Hora Services QA Team  
